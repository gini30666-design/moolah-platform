...
function DiscoverContent() {
  ...
  return (
    <div style={{ background: '#f5efe6', minHeight: '100vh' }}>
      ...
      {/* Sticky Nav */}
      <nav style={{ background: 'rgba(245,239,230,0.92)', borderBottom: '1px solid rgba(166,137,102,0.18)', backdropFilter: 'blur(12px)' }}
        className="sticky top-0 z-40">
        <div className="max-w-md mx-auto flex items-center justify-between px-5 h-14">
          <Link href="/" className="font-display text-base tracking-[.18em] uppercase" style={{ color: 'var(--charcoal)' }}>
            MooLah
          </Link>
          {step !== 'category' && (
            <button
      ...
      {/* Header */}
      <div className="max-w-md mx-auto px-5 pt-8 pb-5">
        <span className="text-[10px] tracking-[0.22em] uppercase block mb-2" style={{ color: 'var(--oak)' }}>
          {step === 'category' ? 'DISCOVER · 探索職人' : step === 'location' ? `${category} · 選擇縣市` : `${category} · ${selectedCity}`}
        </span>
        <h1 className="font-display leading-tight" style={{ fontSize: 'clamp(1.6rem,6vw,2.2rem)', fontWeight: 300, color: 'var(--charcoal)' }}>
          {step === 'category' && '選擇服務類別'}
          {step === 'location' && '您在哪個縣市？'}
          {step === 'results' && (searched ? `找到 ${providers.length} 位職人` : '搜尋中')}
        </h1>
        {step === 'category' && (
          <p className="text-sm mt-2 leading-relaxed" style={{ color: 'rgba(44,40,37,0.55)' }}>
            選擇服務項目，找到所在縣市的優質職人
          </p>
        )}
      </div>
      ...
      {step === 'category' && (
        <div className="max-w-md mx-auto px-5">
          <div className="grid grid-cols-2 gap-3">
            {CATEGORIES.map(cat => (
              <button
                ...
                style={{ height: '200px', borderRadius: '16px', border: '1px solid rgba(166,137,102,0.20)' }}
              >
                ...
                <img
                  ...
                  style={{ opacity: 0.75, filter: 'brightness(.80) saturate(1.05)' }}
                />
                <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(20,16,12,0.90) 40%, transparent 75%)' }} />
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <p className="text-[10px] tracking-[.2em] uppercase mb-1" style={{ color: 'var(--oak)' }}>{cat.no}</p>
          ...
          <div className="mt-10 pt-6 border-t" style={{ borderColor: 'rgba(166,137,102,0.20)' }}>
            <p className="text-xs text-center mb-4" style={{ color: 'rgba(44,40,37,0.45)' }}>您是美業職人？</p>
            <div className="text-center">
              <Link
                href="/join"
                className="inline-block text-xs tracking-widest uppercase px-6 py-3 border hover:opacity-75 transition-opacity"
                style={{ color: 'var(--charcoal)', borderColor: 'rgba(166,137,102,0.35)', borderRadius: '4px' }}
              >
                申請加入 MooLah
              </Link>
            </div>
        ...
        <div className="max-w-md mx-auto px-5">
          ...
          {selectedCatObj && (
            <div
              ...
              style={{ background: 'white', border: '1px solid rgba(166,137,102,0.25)', borderRadius: '12px' }}
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              ...
              <div>
                ...
                <p className="text-sm font-medium" style={{ color: 'var(--charcoal)' }}>{selectedCatObj.label}</p>
              </div>
            </div>
          )}
          ...
          {suggestions.length > 0 && (
            <div className="mb-4 overflow-hidden" style={{ background: 'white', borderRadius: '12px', border: '1px solid rgba(166,137,102,0.18)' }}>
              {suggestions.slice(0, 6).map(s => (
                <button
                  key={s}
                  onClick={() => handleSearch(s)}
                  className="w-full text-left px-4 py-3.5 text-sm border-b last:border-0 hover:opacity-70 transition-opacity"
                  style={{ color: 'var(--charcoal)', borderColor: 'rgba(166,137,102,0.10)' }}
                >
                  {s}
                </button>
              ))}
          ...
          {!cityInput && (
            <div className="mt-5">
              <p className="text-xs mb-3 px-1 tracking-widest uppercase" style={{ color: 'rgba(44,40,37,0.45)' }}>常見縣市</p>
        ...
        <div className="max-w-md mx-auto px-5">
          {loading && (
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              <div className="w-8 h-8 rounded-full border-2 border-[rgba(166,137,102,0.20)] border-t-[#A68966] animate-spin" />
          ...
          {!loading && searched && providers.length === 0 && (
            <div className="text-center py-16 px-4">
              <div
                ...
                style={{ background: 'white', border: '1px solid rgba(166,137,102,0.25)' }}
              >
              ...
              <h2 className="font-display text-xl mb-2" style={{ color: 'var(--charcoal)', fontWeight: 400 }}>此區域尚未開放</h2>
              <p className="text-sm leading-relaxed mb-1" style={{ color: 'rgba(44,40,37,0.55)' }}>
            ...
            <div className="space-y-3">
              {providers.map((p) => (
                <Link
                  ...
                  style={{ background: 'white', borderRadius: '16px', padding: '16px', border: '1px solid rgba(166,137,102,0.14)' }}
                >
                  {/* Avatar */}
                  <div
                    className="w-14 h-14 rounded-full flex-shrink-0 overflow-hidden flex items-center justify-center font-display text-lg"
                    style={{ background: 'rgba(166,137,102,0.12)', color: 'var(--oak)' }}
                  >
                    {p.avatarUrl
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {p.storeName && (
                      <p className="text-xs truncate mt-0.5" style={{ color: 'rgba(44,40,37,0.50)' }}>{p.name}</p>
                    )}
                    <span className="inline-block text-[10px] px-2 py-0.5 mt-1 tracking-wide" style={{ color: 'var(--oak)', background: 'rgba(166,137,102,0.10)', borderRadius: '99px' }}>
                      {p.category}
                    </span>
                    {p.address && (
                      <p className="text-xs mt-1.5 truncate flex items-center gap-1" style={{ color: 'rgba(44,40,37,0.45)' }}>
              ...
              {/* Bottom CTA */}
              <div className="pt-8 pb-4 text-center border-t mt-6" style={{ borderColor: 'rgba(166,137,102,0.18)' }}>
                <p className="text-xs mb-3" style={{ color: 'rgba(44,40,37,0.45)' }}>找不到理想的職人？</p>
                <Link
                  href="/join"
                  className="text-sm tracking-wide pb-0.5 border-b"
...
export default function DiscoverPage() {
  return (
    <Suspense fallback={
      <div className="flex h-screen items-center justify-center text-xs tracking-widest uppercase" style={{ color: 'var(--oak)', background: '#f5efe6' }}>
        載入中
      </div>
    }>
      <DiscoverContent />
