...
const FAQ: { q: string; a: string }[] = [
  ...
  { q: '有儲值功能嗎？', a: '有，儲值金和次卡都能開。但錢不經過 MooLah——客人一樣在店裡付給你（現金、轉帳、你自己的刷卡機都行），系統只負責記帳：餘額多少、扣了幾次、什麼時候到期。客人在 LINE 裡隨時看得到自己的餘額，每次扣款也會自動收到通知。我們不碰金流，所以不抽成，錢也不會卡在平台。' },
  { q: '可以先收訂金，避免客人放鳥嗎？', a: '目前不經手線上收款，但系統會自動記住每位客人未到店的次數，累計 3 次就擋下他的線上預約，這個規則也會顯示給客人看。如果你想收訂金，可以請客人轉帳到你自己的帳戶再由你確認，錢一樣直接進你口袋。' },
  { q: '開儲值卡會不會有法律問題？', a: '預收款在台灣有《美容定型化契約應記載及不得記載事項》的規範，義務在店家身上（和你現在用紙卡記次是一樣的）。MooLah 的做法是把該填的欄位內建進去：有效期限、退費規則、客人確認存證；贈送比例超過 20%、單筆收款達 1 萬、未使用餘額逾 5 萬時系統會提醒你。我們提供提醒與紀錄，不提供法律意見，細節建議與你的會計師確認。' },
...
{/* ⚠️ 定位鐵律（2026-08-03 Gini 拍板）：人眼讀到的第一句**不放「預約系統」**。
    ...
    「0 抽佣・不綁約・免下載」下面的 chips 已經在講了，段落裡重複只是稀釋。 */}
        ...
        */}
        ...
        {/* ══════════ 儲值卡／次卡 ══════════
            2026-08-11 加：約 1/3 的招商進線「第一句」就問有沒有儲值，是購買門檻不是加分項。
            寫法上刻意先講「錢不經過我們」——職人對儲值最在意的不是功能，是錢會不會被卡住。
            ⚠️ 誠實邊界：我們只做帳本，平台代收儲值款項＝電子支付業務（做不到也不該做）。
                這裡的每一句都必須對得上 lib/credits.ts 真的做出來的東西。 */}
        <section data-animate style={{ background: cream, padding: '52px 22px 56px' }}>
          <div style={{ maxWidth: '620px', margin: '0 auto' }}>
            <p style={{ fontSize: '12px', color: '#8a6f4f', fontWeight: 700, marginBottom: '12px' }}>儲值・次卡・訂金</p>
            <h2 style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 'clamp(1.9rem, 7vw, 2.6rem)', fontWeight: 400, color: charcoal, lineHeight: 1.25, marginBottom: '14px' }}>
              客人的儲值金，<br />直接進<span style={{ color: oak }}>你的口袋</span>
            </h2>
            <p style={{ fontSize: '14.5px', lineHeight: 1.85, color: 'rgba(44,40,37,0.8)', marginBottom: '28px' }}>
              儲值金和次卡都能開。錢還是像現在一樣，客人在店裡付給你——現金、轉帳、你自己的刷卡機都行。
              <strong style={{ color: charcoal }}>MooLah 不經手任何一塊錢，也不抽成</strong>，只負責把帳記清楚。
            </p>

            <div style={{ display: 'grid', gap: '10px', marginBottom: '26px' }}>
              {[
                { t: '客人自己看得到餘額', d: '每次扣款，客人的 LINE 會自動收到一張卡：扣了多少、還剩多少。不用再翻本子核對，也不會有「上次到底扣多少」的爭議。' },
                { t: '紀錄改不掉、刪不掉', d: '每一筆儲值和扣款都永久留著。按錯了只能新增一筆「更正」，原紀錄照樣看得到——真的有爭議時，每一塊錢都查得到來歷。' },
                { t: '幫你把法規顧好', d: '開卡時就會填有效期限和退費規則，客人確認後留存證。贈送比例超過 20%、單筆收款破萬、未使用餘額逾 5 萬時，系統會提醒你——這些都是《美容定型化契約》的規定，多數人不知道。' },
              ].map(x => (
                <div key={x.t} style={{ padding: '18px', background: '#ffffff', border: '1px solid rgba(166,137,102,0.22)', borderRadius: '14px' }}>
                  <p style={{ fontSize: '15px', fontWeight: 700, color: charcoal, marginBottom: '5px' }}>{x.t}</p>
                  <p style={{ fontSize: '13.5px', lineHeight: 1.78, color: 'rgba(44,40,37,0.72)' }}>{x.d}</p>
                </div>
              ))}
            </div>

            <p style={{ fontSize: '13.5px', lineHeight: 1.8, color: 'rgba(44,40,37,0.7)', paddingLeft: '14px', borderLeft: `2px solid ${oak}` }}>
              <strong style={{ color: charcoal, fontWeight: 700 }}>怕放鳥？</strong>
              系統會記住每位客人未到店的次數，累計 3 次就自動擋下他的線上預約——
              不用先跟客人收訂金，也不用你自己記在腦袋裡。
            </p>
          </div>
        </section>
